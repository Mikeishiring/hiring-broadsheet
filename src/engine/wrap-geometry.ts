// Ported from chenglou/pretext pages/demos/wrap-geometry.ts
// Polygon hull extraction, interval carving, and hit testing for obstacle avoidance.
...
export function transformWrapPoints(points: readonly Point[], rect: Rect, angle: number): Point[] {
  if (angle === 0) {
...
export function isPointInPolygon(points: readonly Point[], x: number, y: number): boolean {
  let inside = false
...
export function getCircleIntervalForBand(
  ...
  const top = cy - radius - verticalPadding
  const bottom = cy + radius + verticalPadding
  if (bandBottom <= top || bandTop >= bottom) return null

  const bandMid = (bandTop + bandBottom) / 2
  const dy = bandMid - cy
  const r = radius + horizontalPadding
...
function getPolygonXsAtY(points: readonly Point[], y: number): number[] {
  const xs: number[] = []
...
function makeConvexHull(points: readonly Point[]): Point[] {
